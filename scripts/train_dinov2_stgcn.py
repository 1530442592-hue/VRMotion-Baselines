...
from common.utils import get_device, get_logger # 移除 make_output_dir，使用自带 os 路径管理
from models.dinov2_stgcn import DINOv2STGCN
...
# =================配置区=================
DEVICE = get_device()
MODEL_NAME = "dinov2_stgcn_100ep_fixed" # 正确的 DINOv2 名字
EPOCHS = 100  
BATCH_SIZE = 2 # STGCN 耗显存，如果 OOM 则改为 1
LR = 5e-5 # 🌟 对齐真 ST-GCN 学习率
TEST_RATIO = 0.2
...
# =================原生视觉预处理 (DINOv2 标准ImageNet参数)=================
logger.info("⚡ 使用原生 PyTorch 视觉预处理管道 (ImageNet标准)...")
...
# =================模型与优化器初始化=================
logger.info("🧠 正在加载 DINOv2-Giant (11亿参数) + STGCN 模型...")
model = DINOv2STGCN(seq_len=16, pred_len=16, num_joints=24, use_8bit=False).to(DEVICE)
