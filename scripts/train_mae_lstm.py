...
from models.videomae_lstm import VideoMAELSTM

# =================核心魔法：全方位锁定随机种子=================
def set_seed(seed=42):
    """
    固定所有的随机种子，确保实验 100% 可复现，与 ResNet 完全对齐
    """
    random.seed(seed)
    np.random.seed(seed)
...
# =================配置区（与 ResNet 完全对齐）=================
DEVICE = get_device()
MODEL_NAME = "videomae_lstm_100ep_fixed"  # 新名字以区分旧实验
EPOCHS = 100
BATCH_SIZE = 32  # 【关键修改】：与 ResNet 对齐（原来是 8）
LR = 1e-4
TEST_RATIO = 0.2
...
# =================标准预处理 (ImageNet，与 ResNet 完全一致)=================
logger.info("⚡ 使用标准 ImageNet 视觉预处理管道（与 ResNet 对齐）...")
...
def vlm_transform(frames_np):
    """将帧列表转换为标准化的 tensor（与 ResNet 完全一致）"""
...
# 因为锁了种子，这里切分出来的测试集和 ResNet 会一模一样
...
# =================模型初始化=================
logger.info("🧠 正在加载 VideoMAE + LSTM 模型架构（已修复，与 ResNet 对齐）...")
model = VideoMAELSTM(seq_len=16, pred_len=16, num_joints=24).to(DEVICE)
...
logger.info(f"🚀 开始 100 轴训练 {MODEL_NAME} (VideoMAE 修复版，与 ResNet 公平对比)...")
