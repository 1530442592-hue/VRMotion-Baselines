...
from models.videomae_stgcn import VideoMAESTGCN
...
# =================配置区=================
DEVICE = get_device()
MODEL_NAME = "videomae_stgcn_100ep_fixed"
EPOCHS = 100
BATCH_SIZE = 8 # ⚠️ 如果显存 OOM，请降为 2 或 1
LR = 5e-5 # 🌟 关键对齐：降低学习率，保护图结构
TEST_RATIO = 0.2
...
dataset = BeatSaberDataset(data_root="data/beat_saber") # VideoMAE 自带 transform，所以这里不需要传
...
model = VideoMAESTGCN(seq_len=16, pred_len=16, num_joints=24).to(DEVICE)
...
for epoch in range(EPOCHS):
    ...
    te_fps = calculate_fps(time.time() - start_time_test, len(test_dataset))
